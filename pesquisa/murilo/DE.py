...
# crossover_ratio = [0,1]
def DE(objf,lb,ub,dim,PopSize,iters,trainInput,trainOutput,net):
    ...
    for i in range(PopSize):
        fitness = objf(population[i, :],trainInput,trainOutput,net)
    ...
    while t < iters:
        ...
        for i in range(PopSize):
            ...
            # calc fitness
            mutant_fitness = objf(mutant_sol, trainInput,trainOutput,net)
            #s.func_evals += 1
    ...
    s.bestIndividual=numpy.asarray(s.leader_solution)
    # return solution
    return s
